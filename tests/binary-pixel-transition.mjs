...
assert.match(pixelGrid, /DESKTOP_CELL_SIZE\s*=\s*13/, 'desktop binary cells balance density and rendering cost')
...
assert.match(pixelGrid, /MIN_DURATION\s*=\s*1400/, 'transition completes promptly')
assert.match(pixelGrid, /MAX_DURATION\s*=\s*1400/, 'transition duration stays deterministic and synchronized')
