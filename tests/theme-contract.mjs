...
assert.match(pixelTransition, /aria-hidden="true"/, 'transition decoration is hidden from assistive tech')
assert.match(pixelTransition, /Math\.random\(\) > 0\.5 \? 1 : 0/, 'generates random binary glyphs')
...
assert.match(css, /binary-pixel-transition/, 'binary glare transition is styled')
