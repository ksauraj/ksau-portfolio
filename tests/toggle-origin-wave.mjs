...
assert.match(navbar, /onClick=\{\(event\).*toggleTheme\(event\.currentTarget/s, 'toggle passes its button as the ripple origin')
assert.match(provider, /origin:\s*\{\s*x:\s*number;\s*y:\s*number\s*\}/, 'provider stores a viewport-space ripple origin')
assert.match(provider, /getBoundingClientRect/, 'provider derives the ripple origin from the clicked toggle')
assert.match(transition, /Math\.hypot/, 'canvas renders by radial distance from the toggle')
assert.match(transition, /distanceToWave/, 'binary glyphs form an expanding circular ring')
assert.match(transition, /distanceToWave < 0 \|\| distanceToWave > waveWidth/, 'binary crest trails the newly revealed background')
